...
namespace AI_stats_measurement.Backend.Services.Parsing
{
    public static class TextCleaner
    {
        public static string Clean(string text)
        {
            ...
            text = Regex.Replace(text, @"\b(tabel|table)\s*\(?\s*\d+[A-Z]+\s*\)?", "", RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(100)); // Remove patterns like "tabel 1A"
